...
exports.createBooking = catchAsync(async(req,res)=>{
  ...
  const existingBooking = await Booking.findOne({ classId, userId: req.user.userId });
  if (existingBooking) {
    next (new ApiError('You have already booked this class',404));
  }

  const booking = await Booking.create({ classId, userId: req.user.userId });
  res.status(200).json({
    status:'success' ,
    data :{
        data :booking
...
exports.getBooking = catchAsync(async(req,res,next)=>{
    const bookings = await Booking.find({ userId: req.user.userId }).populate('classId');
    if(!bookings) {
